...
|---|---|---|---|
| **File sources** | `src/sources.ts` *(new)* | `collectFilesFromLocal({cwd, mode})` — `changed` (default) = `git diff --name-only <merge-base default>` ∪ `git status --porcelain` (uncommitted); `full` = `git ls-files`. Filters via `hasCodeExtension`. Reads via `node:fs/promises`. Also houses `collectFilesFromGitHub()` extracted from today's `auditRepo`. Git invoked through a small injectable runner so tests stub output. | `git`, `hasCodeExtension` |
