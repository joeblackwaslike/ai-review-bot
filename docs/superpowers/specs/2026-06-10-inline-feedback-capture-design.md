...
| `src/feedback/reactions.ts` | create | Given an octokit + a stored comment record + its last-seen reaction state, fetch current reactions and compute the **new/changed verdict events** (pure diff logic over an injected client). Exports: `diffReactions(octokit, record): Promise<FeedbackEvent[]>` and a pure `computeReactionDelta(current, lastSeen)`. |
