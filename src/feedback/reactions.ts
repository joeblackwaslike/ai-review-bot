...
// A Map, not an object literal: `content` is an unconstrained string, and plain
// object indexing walks the prototype chain — `content: "toString"` would return
// a truthy function and be emitted as a verdict.
const VERDICT_BY_REACTION = new Map<string, Verdict>([
	["+1", "up"],
	["-1", "down"],
	["confused", "confused"],
]);
...
export function computeReactionDelta(
	...
	for (const r of current) {
		const verdict = VERDICT_BY_REACTION.get(r.content);
		if (!verdict) continue;
